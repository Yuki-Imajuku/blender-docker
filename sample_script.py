...
class BlenderDatasetGenerator:
    ...
    def __init__(self, dataset_params: Optional[Dict[str, Tuple[int, bool]]]):
        ...
        if dataset_params is None:
            dataset_params = {
                "train": (self.TRAIN_VIEWS, False),
                "val": (self.VAL_VIEWS, False),
                "test": (self.TEST_VIEWS, True),
            }
    ...
    def generate_data_split(
        ...
        for i in range(0, num_images):
            ...
            if self.RANDOM_VIEWS:
                scene.render.filepath = self.fp + f"/{data_split}/r_{i:03d}"
                if self.UPPER_VIEWS:
                ...
                scene.render.filepath = self.fp + f"/r_{i:03d}"
            ...
            frame_data = {
                "file_path": f"./{data_split}/r_{i:03d}",
    ...
    def enable_gpu():
        ...
        for device in bpy.context.preferences.addons["cycles"].preferences.devices:
            device.use = True
            # print(f"Device '{device.name}' type {device.type}: {device.use}")
